...
for machines in macine_list:
    ...
    for subdir in dirs:
        ...
        for files in subpath:
            ...
            # if files == "ibmq_5_yorktown - ibmqx2.csv":
                ...
                T1_Model = ExponentialSmoothing(T1_data)
                T1_Fit = T1_Model.fit(optimized=True)
                Yhat_T1 = T1_Fit.predict(len(T1_data), len(T1_data))
                Final_T1.append(Yhat_T1)

                T2_Model = ExponentialSmoothing(T2_data)
                T2_Fit = T2_Model.fit(optimized=True)
                Yhat_T2 = T2_Fit.predict(len(T2_data), len(T2_data))
                Final_T2.append(Yhat_T2)

                Frequency_Model = ExponentialSmoothing(Freqency_data)
                Freqency_Fit = Frequency_Model.fit(optimized=True)
                Yhat_Frequency = Freqency_Fit.predict(len(Freqency_data), len(Freqency_data))
                Final_Frequncy.append(Yhat_Frequency)

                Readout_Model = ExponentialSmoothing(Readout_data)
                Readout_Fit = Readout_Model.fit(optimized=True)
                Yhat_Readout = Readout_Fit.predict(len(Readout_data), len(Readout_data))
                Final_Readout.append(Yhat_Readout)

                SQU3_Model = ExponentialSmoothing(SQU3_data)
                SQU3_Fit = SQU3_Model.fit(optimized=True)
    ...
    plt.boxplot(Original_T1, boxprops=boxprops)
    plt.boxplot(Original_T2, boxprops=boxprops)
    ...
    plt.legend()
    # x = np.linspace(Original_Frequency)
    ...
    plt.xlabel('Temporal Difference')
    plt.ylabel('Operational Frequency')
